...
def parse_workflow(source) -> tuple[Workflow | None, Diagnostics]:
    ...
    activities, arcs, precedence, used = _expand_body(entry, entry_proc, procs, atomic, diags)
...
def _expand_body(entry_name, entry_proc, procs, atomic, diags):
    ...
    # The entry's own inputs (if any) have no in-body producer, so `inputs.*` inside
    # it resolves to nothing. The entry name seeds the recursion guard's stack.
    exp.expand(entry_proc, (), {}, (entry_name,))
    ...
    return exp.activities, exp.arcs, precedence, exp.used
