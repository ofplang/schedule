...
with the workflow provenance (`node` / `arc`) carried alongside. Every activity
is `pending` on a plan, so `status` is omitted; `now` and `placements` belong to
a replanning status and are not emitted here.
"""
...
def render_plan(
    ...
    *,
    ...
    doc: dict = {
        "time": {"unit": instance.time_unit},
        "outcome": solution.outcome,
        "objective": {"kind": "makespan", "value": solution.makespan},
