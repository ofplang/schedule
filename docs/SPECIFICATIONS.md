...
| `interchangeable_resources` | **warning**: this instance offers several interchangeable ways of using one resource — spots of a device, devices of a pool, transporters that can make the same moves — so it carries modes and routes that only choose between them, and every such choice leads to the same schedule. Nothing is wrong with the laboratory or the document: the model is simply larger than the question needs, and model size is what bounds solve time. One diagnostic per class, naming its members and how many modes and routes the choice costs. Reported only where the choice costs something — resources nothing chooses between are interchangeable in the same sense and say nothing. Interchangeability is claimed **of this instance**, not of the environment: only the processes the workflow instantiated and the routes its arcs kept are compared, and a resource the document has pinned something to (reported history, a boundary binding §6.8, an `occupied` hold §6.12) is never claimed, so a class shrinks as a run accumulates history. **Nothing is done about it** — no constraint is added and no plan differs |
