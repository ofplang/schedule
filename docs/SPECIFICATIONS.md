...
| `interchangeable_resources` | **warning**: this instance offers several interchangeable ways of using one resource — spots of a device, devices of a pool, transporters that can make the same moves — so it carries modes and routes that only choose between them, and every such choice leads to the same schedule. Nothing is wrong with the laboratory or the document: the model is simply larger than the question needs, and model size is what bounds solve time. One diagnostic per class, naming its members and how many modes and routes the choice costs. Interchangeability is claimed **of this instance**, not of the environment: only the processes the workflow instantiated and the routes its arcs kept are compared, and a resource the document has pinned something to (reported history, a boundary binding §6.8, an `occupied` hold §6.12) is never claimed, so a class shrinks as a run accumulates history. Two kinds of class are **not** reported, both because nothing is being paid for them: one nothing chooses between, and one the scheduler **collapses** — a class of interchangeable *transporters* becomes one resource of capacity `len(members)`, with the arm that makes each move named after the solve. That collapse cannot change the schedule (see FORMULATION, CP-SAT implementation notes) and is not applied where it could: a class one of whose routes takes no time (§5.4) is reported like any other. For every other scope nothing is done — no constraint is added and no plan differs |
