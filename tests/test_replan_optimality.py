...
def test_running_source_clamped_by_now_and_margin(tmp_path):
    ...
    status = (
        ...
        "now: 5\n"
        "activities:\n"
        "- { kind: processing, status: running, start: 0, end: 1, process: source, "
        "mode: '0', node: [SampleSource], output_spots: { source_out: station_0.core } }\n"
    )
    s = write(tmp_path, "s.yaml", status)
    assert (
        schedule(SIMPLE_WF, env, document_path=s, running_task_margin=0).makespan == 8
    )  # end 5 + 1 + 2
    assert (
        schedule(SIMPLE_WF, env, document_path=s, running_task_margin=3).makespan == 11
    )  # end 8 + 1 + 2
...
def test_idempotent_all_completed_reoptimises_nothing(tmp_path):
    ...
    status = (
        ...
        "from_spot: station_0.core, to_spot: station_1.core, transporter: transport, "
        "arc: { from: { node: [SampleSource], port: source_out }, "
        "to: { node: [SampleTarget], port: target_in } } }\n"
        "- { kind: processing, status: completed, start: 3, end: 5, process: target, "
        "mode: '0', node: [SampleTarget], input_spots: { target_in: station_1.core } }\n"
    )
    report = schedule(SIMPLE_WF, env, document_path=write(tmp_path, "s.yaml", status))
...
def test_stays_when_arrival_mode_still_available(tmp_path):
    ...
    env = write(tmp_path, "env.yaml", st_env(**_LINE))
    report = schedule(
        SIMPLE_WF, env, document_path=write(tmp_path, "s.yaml", committed_source_and_leg(now=5))
    )
...
def test_chained_reroute_makespan(tmp_path):
    ...
    status = (
        ...
        "from_spot: station_1.core, to_spot: station_2.core, transporter: transport, "
        "arc: { from: { node: [SampleSource], port: source_out }, "
        "to: { node: [SampleTarget], port: target_in } } }\n"
    )
    report = schedule(SIMPLE_WF, env, document_path=write(tmp_path, "s.yaml", status))
...
def test_bounce_makespan_and_round_trip(tmp_path):
    ...
    status = (
        ...
        "from_spot: station_2.core, to_spot: station_1.core, transporter: transport, "
        "arc: { from: { node: [SampleSource], port: source_out }, "
        "to: { node: [SampleTarget], port: target_in } } }\n"
    )
    report = schedule(SIMPLE_WF, env, document_path=write(tmp_path, "s.yaml", status))
    # target back on station_1; folding leaves the solved makespan unchanged
    assert report.makespan == 22
...
_MULTI_STATUS = (
    ...
    "to_spot: dt.a, transporter: transport, "
    "arc: { from: { node: [S1], port: o }, to: { node: [M], port: i1 } } }\n"
    "- { kind: transport, status: completed, start: 2, end: 3, seq: 0, from_spot: d2.s, "
    "to_spot: dt.b, transporter: transport, "
    "arc: { from: { node: [S2], port: o }, to: { node: [M], port: i2 } } }\n"
)
...
def test_multi_input_infeasible_when_no_reachable_mode(tmp_path):
    ...
    env = _MULTI_ENV.replace(
        "  merge:   { modes: [ { devices: [dt], duration: 2, "
        "input_spots: { i1: dt.a, i2: dt.b } } ] }",
        "  merge:   { modes: [ { devices: [dz], duration: 2, "
        "input_spots: { i1: dz.p, i2: dz.q } } ] }",
    ).replace(
        "  - { id: dt, spots: [a, b] }",
        "  - { id: dt, spots: [a, b] }\n  - { id: dz, spots: [p, q] }",
    )
    wf = write(tmp_path, "wf.yaml", MULTI_INPUT_WF)
    report = schedule(
        wf, write(tmp_path, "env.yaml", env), document_path=write(tmp_path, "s.yaml", _MULTI_STATUS)
    )
